...
from tutor.commands.dev import DevJobRunner
from tutor.commands.local import LocalJobRunner


@hooks.Actions.COMPOSE_PROJECT_STARTED.add()
def _stop_other_projects(root: str, config, project_name: str) -> None:
    """
    ...
    with hooks.contexts.enter("stop-other-projects"):
        ...
        def _replace_tutor_app(template_vars):
            template_vars_without_tutor_app = [
                (name, value) for name, value in template_vars
                if name != "TUTOR_APP"
            ]
    ...
    LocalJobRunner(root, config_to_stop).docker_compose("stop")
    DevJobRunner(root, config_to_stop).docker_compose("stop")
